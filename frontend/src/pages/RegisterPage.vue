    ...
    <div class="auth-wrap">
      ...
      <div class="auth-right">
        <div class="auth-form">
          ...
          <div class="auth-sub">Register to access CardioTriage</div>
          <form @submit.prevent="handleSubmit">
            <div class="form-group"><div class="form-label">Email address</div><input v-model="email" class="form-input" type="email" placeholder="your@email.com" required></div>
            <div class="form-group"><div class="form-label">Password</div><input v-model="password" class="form-input" type="password" placeholder="Minimum 8 characters" required minlength="8"></div>
            <div class="form-group"><div class="form-label">Confirm password</div><input v-model="confirmPassword" class="form-input" type="password" placeholder="Repeat your password" required></div>
            <p v-if="error" style="color:var(--red-text);font-size:12px;margin-bottom:12px;">{{ error }}</p>
            <button class="btn primary full" type="submit" :disabled="loading" style="margin-top:4px">{{ loading ? 'Creating account…' : 'Create account' }}</button>
          </form>
          <div class="auth-lnk">Already have an account? <span @click="router.push('/login')">Sign in →</span></div>
        </div>
      </div>
    </div>
  </PublicLayout>
